...
https://github.com/stefanos50/thesis_test_repository/assets/36155283/0ac23215-3889-431b-96cd-4bf94f7936b2
...
For training CARLA on another target dataset, we provide a synthetic dataset containing CARLA frames with their corresponding G-Buffers and Ground Truth Labels, available [**here**](https://drive.google.com/drive/folders/1vyo71eW7F0r0_EU60bf99QCrak0IU5ga?usp=sharing). Robust labels generated from [MSEG](https://github.com/mseg-dataset/mseg-semantic) are also provided. Essential scripts for training are located in `\code\epe\dataset_generation`.

The dataset is presented in an unprocessed state to allow usability for various other models. Utilize `GBuffersPreprocess.py` and `LabelMapPreprocess.py` to preprocess the ground truth labels into class masks and stack the G-Buffers. If you prefer to create your own synthetic dataset, the `generate_dataset.py` script can be employed to generate a dataset in synchronous mode, ensuring that all data is exported synchronized from the same frame ID.

We recommend utilizing the dataset in the specific structure shown below. This is because the `\dataset_generation\generate_fake_txt.py` script, responsible for generating the required TXT files containing all the dataset paths, expects the data to be organized in this particular format. In any other scenario, you will need to modify or create your own script for this task.
...
We also offer some pre-defined TXT files in the `\config\data\Carla` directory, which includes all the dataset paths, and `\config\data\Carla_Original`, containing a subset of the dataset (15,000 frames) excluding night-time frames. If you are using a disk with a different letter, run `\dataset_generation\update_txt.py` to update these files accordingly.
...
> 💡 **Hint**: Before running any of the scripts in `\code\epe\dataset_generation`, use the `--help` argument to check the available parameters and obtain information about how to use them.
...
> ⚠️ **Warning**: Parked vehicles are not considered actors, and their extraction of bounding boxes is impossible via the Python API. In that case, use the Opt version of the selected town (Town10HD_Opt, Town01_Opt, etc.), and our approach will remove all the parked vehicles from the world. Moreover, the Cybertruck vehicle seems to have issues with the bounding boxes, so we recommend filtering it for object detection dataset generation.
